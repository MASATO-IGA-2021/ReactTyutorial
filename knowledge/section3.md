...
- props を利用する
- children から目的の要素を取り出す

#### 3-3-3 children に対してパラメーターを引き渡す

- props.children プロパティが表すコンテンツは、あくまで呼び出し元のスコープなので、コンポーネント配下の情報にアクセスすることはできない。
...
- State に新しい値が反映するのはイベントハンドラーを終えた後

#### 3-3-6 子コンポーネントから親コンポーネントへの情報伝達

- 親コンポーネントで State を更新する処理を実装し、子コンポーネントでは任意のタイミングで親コンポーネントの処理を呼び出すようにすることで親コンポーネントで集中的に管理を行うことができる。
...
- マウスの出入りに応じて画像を入れ替える
- mouseenter/mouseleave/mouseover/mouseout の相違点
...
- キーイベントでのキーを識別する
- イベントハンドラーに任意の引数を渡す
  - onEvent 属性に渡すのは、**関数呼び出しではなく**関数そのものでなくてはいけない。
- 独自データ属性を利用する
  - **独自データ属性**:任意タグに data-xxxxx の形式で指定できる属性
    - 小文字の英数字
    - ハイフン(-)
    - ドット(.)
    - コロン(:)
...
- イベントが目的要素に達するまでの過程
  - **キャプチャフェーズ**：最上位の windows オブジェクトから下位の要素にイベントを伝播するフェーズ →
...
- 伝播を抑制する
  - `e.stopPropagation();`
- イベント既定の動作をキャンセルする
  - `preventDefault`メソッドを利用し処理の動作をキャンセルする

#### 3-4-4 イベントハンドラーのオプションを設定する

| 既定値  | 概要                                 |
| ------- | ------------------------------------ |
| capture | キャプチャフェーズでハンドラーを実行 |
| once    | 初回イベントでのみハンドラーを実行   |
| passive | Passive モードで実行                 |

- 非 Passive モードでイベントハンドラーを設定する
  - ハンドラーに紐づくメソッドはハンドラー内で preventDefault されないことを随時確認しながら処理することで動作遅延の可能性が出てきます。
    　そこで preventDefault メソッドを呼び出さない場合あらかじめブラウザに対して通知する。(Passive モード)
